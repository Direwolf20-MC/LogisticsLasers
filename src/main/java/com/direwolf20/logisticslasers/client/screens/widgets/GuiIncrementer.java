...
public class GuiIncrementer extends Widget {
    ...
    public GuiIncrementer(int x, int y, int min, int max, int w, int h, @Nullable IIncrementerChanged onChange) {
        ...
        this.minusButton = new DireButton(this.x, this.y - 1, 12, h + 2, new StringTextComponent("-"), (button) -> this.updateValue(true));
        this.field = new GuiTextFieldBase(Minecraft.getInstance().fontRenderer, x + 13, y, w, h).setDefaultInt(this.value).restrictToNumeric();
        this.plusButton = new DireButton(this.x + w + 14, this.y - 1, 12, h + 2, new StringTextComponent("+"), (button) -> this.updateValue(false));
